...
test.describe('Form Layouts page', () => {
    ...
    test('input fields', async({page}) => {
        ...
        await usingTheGridEmailInput.pressSequentially('test2@test.com', {delay: 100}); // delay allows us to type text with time delay
        ...
        //locator assertion
        await expect(usingTheGridEmailInput).toHaveValue('test2@test.com') // should to use toHaveValue for input field (not toHaveText)
    })
    ...
    test.only('radio buttons', async({page}) => {
        ...
        const radioStatus = await usingTheGridEmailInput.getByRole('radio', {name: "Option 1"}).isChecked(); // return true or false
        //generic assertion
...
test('list and dropdowns', async({page}) => {
    ...
    // const optionList = page.getByRole('list').locator('nb-option'); //return all list items
    const optionList = page.locator('nb-option-list nb-option'); //return all list items
